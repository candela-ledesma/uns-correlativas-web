...
\subsubsection*{PlanSeleccionado}
...
\textbf{Limitación conocida:} la clave foránea \texttt{careerId} apunta a \texttt{Carrera},
no a \texttt{PlanVersion}. Esto significa que no se registra \emph{qué versión específica}
del plan seleccionó el usuario cuando una carrera tiene más de una versión disponible.
El nombre \texttt{PlanSeleccionado} sugiere selección de plan, pero la granularidad real
es a nivel de carrera. Una migración futura debería agregar una FK opcional a
\texttt{PlanVersion} para soportar selección por versión.
