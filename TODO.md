...
- `media` [x] **Prompt genérico para otras universidades** — `GENERIC_SYSTEM_PROMPT` en `lib/ai/prompt.ts`, editable desde ConfigTab. En `CargarPlanTab`, al seleccionar "Otra universidad" se usa el prompt genérico. Ambos prompts persisten en `AdminConfig` (columna `genericPrompt` en Neon, migración aplicada).
